...
enum Formula[+A]:
    ...
    case Forall(x: Formula[A], p: Formula[A])
    case Exists(x: Formula[A], p: Formula[A])
...
object Formula:
    ...
    */
    ...
    def mkForall[A](x: Formula[A], p: Formula[A]): Formula[A] = Forall(x, p)
    def mkExists[A](x: Formula[A], p: Formula[A]): Formula[A] = Exists(x, p)
